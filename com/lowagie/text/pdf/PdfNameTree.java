    ...
    public static PdfDictionary writeTree(HashMap hashMap, PdfWriter pdfWriter) throws IOException {
        ...
        if (arrobject.length <= 64) {
            ...
            for (int i = 0; i < arrobject.length; ++i) {
                pdfArray.add(new PdfString((String)arrobject[i], null));
        ...
        for (n2 = 0; n2 < arrpdfIndirectReference.length; ++n2) {
            ...
            PdfDictionary pdfDictionary = new PdfDictionary();
            PdfArray pdfArray = new PdfArray();
            pdfArray.add(new PdfString((String)arrobject[n], null));
        ...
        do {
            ...
            for (n3 = 0; n3 < n; ++n3) {
                ...
                PdfDictionary pdfDictionary = new PdfDictionary();
                PdfArray pdfArray = new PdfArray();
                pdfArray.add(new PdfString((String)arrobject[n3 * n4], null));
                pdfArray.add(new PdfString((String)arrobject[Math.min((n3 + 1) * n4, arrobject.length) - 1], null));
                pdfDictionary.put(PdfName.LIMITS, pdfArray);
                pdfArray = new PdfArray();
    ...
    private static void iterateItems(PdfDictionary pdfDictionary, HashMap hashMap) {
        ...
        if (pdfArray != null) {
            ...
            while (n < arrayList.size()) {
                PdfString pdfString = (PdfString)PdfReader.getPdfObjectRelease((PdfObject)arrayList.get(n++));
                hashMap.put(PdfEncodings.convertToString(pdfString.getBytes(), null), arrayList.get(n));
                ++n;
            }
            return;
        }
